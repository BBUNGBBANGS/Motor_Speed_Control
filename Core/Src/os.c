...
void Os_Handler_5ms(void)
{
	if(Os_Var.Os_Counter.Os_5ms_Counter % 1 == 0)
	{
		Os_Var.Os_Counter.Os_5ms_Counter++;
		Os_Var.Os_Task_Flag.Os_5ms_Flag = 1;
	}

	void(*fp)(void);
	if(Os_Var.Os_Task_Flag.Os_5ms_Flag == 1)
	{
		fp = Os_5ms_Task;
		Os_Var.Os_Task_Add.Os_5ms_Address = fp;
		Os_Var.Os_Task_Flag.Os_5ms_Flag = 0;
		Os_Calc_Runtime_Start(OS_TASK_INDEX_5MS);
		(void)Os_5ms_Task();
		Os_Calc_Runtime_Stop(OS_TASK_INDEX_5MS);
		Os_Calc_Period(OS_TASK_INDEX_5MS);
	}
}
